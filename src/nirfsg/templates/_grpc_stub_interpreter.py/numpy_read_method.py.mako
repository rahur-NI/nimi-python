...
<%
    ...
    full_func_name = f['interpreter_name'] + method_template['method_python_name_suffix']
        numpy_complex_params = [p for p in helper.filter_parameters(parameters, helper.ParameterUsageOptions.NUMPY_PARAMETERS) if p.get('complex_array_representation') is not None]
